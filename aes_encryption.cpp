...
{
    word empWord(0x00000000);
    word temp;
    temp = k1.to_ulong();
    temp<<=24; //first 8 bit
    empWord|=temp;  // OR kore add
    temp = k2.to_ulong();
    temp<<=16;
    empWord|=temp;
    temp = k3.to_ulong();
...
word Rconstant[10] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
...
void MixColumns(byte state[16])
{
    ...
    for(int i=0;i<16;i+=4)
    {
        for(int j=0;j<4;j++)
        {
            arr[j] = state[i+j]; //i = column, i+j*4 = row
        }
...
SubBytes(state);
    ...
    for(int i=0;i<4;i++)
        {
            roundKey[i] = Key[4*(Numofround+i)];
        }
    addRoundKey(state,roundKey);
}
void KeyExpansion(byte key[16], word expandedKey[44])
{
    word temp;
    int i=0;
    while(i<Numofkey)
    {
        expandedKey[i] = Word(key[4*i], key[(4*i)+1], key[(4*i)+2], key[(4*i)+3]);
        i++;
    }
    i = Numofkey;

    while(i<4*(Numofround+1))
    {
        ...
        if(i%Numofkey==0)
        {
            word S = lsword(temp);
            S = Subword(S);
            expandedKey[i]= expandedKey[i-Numofkey]^S^Rconstant[i/Numofkey-1];
        }
        else
        {
            expandedKey[i] = temp^expandedKey[i-Numofkey];
        }
        i++;
    }
